...
class EmgData(BaseData):
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        all_df, labels_df = self.load_single(input_paths[0].split('.pkl')[0])  # a single file contains dataset

        return all_df, labels_df

    def load_single(self, data_path_without_extention):

        df = pd.read_pickle(data_path_without_extention + '.pkl')
        labels = np.load(data_path_without_extention + '.npz')['data']
        labels = labels - 1
        labels = pd.Series(labels)
        df_labels = pd.DataFrame(labels.values)
        self.class_names = [f"act_{i}" for i in range(1, 7)]
